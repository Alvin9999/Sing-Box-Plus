...
| 版本    | 日期     | 变更 |
|--------|----------|------|
| v2.1.7-8 | 2025-09 | - 首次运行：自动安装并自检核心依赖<br>- 后续运行：检测到核心命令齐全 → 跳过依赖安装，直接进入脚本菜单。 |
| v2.1.6 | 2025-09 | - WARP 解析与保底：去引号/回车/空格，Reserved 缺失自动 `0,0,0`，一键清理坏值并重建<br>- SS2022 KEY 生成保证标准 Base64（避免“psk: illegal base64…”）<br>- 更换端口后一次回车返回菜单<br>- 菜单配色与“脚本更新地址”同色显示优化 |
