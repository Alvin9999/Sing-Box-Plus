...
# Version: v2.0.3
# 变更：
# - 修复：WARP 启动解析失败导致服务退出（DoH 预解析为 IP + 配置 dns 块）
# - 保留：1.12.x 兼容旧 WG 出站、正确 ExecStart、端口放行、友好提示等
# =======================================================
set -euo pipefail
...
install_wgcf(){ [[ -x "$WGCF_BIN" ]] && return 0
...
write_config(){
...

  {
    ...
    dns:{ servers:[ "https://1.1.1.1/dns-query", "tls://dns.google" ], strategy:"prefer_ipv4" },
    inbounds:[
