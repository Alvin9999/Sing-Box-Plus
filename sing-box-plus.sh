...
HY2_OBFS=${HY2_OBFS:-true}      # 开启 hysteria2 salamander 混淆
...
ensure_unique_ports(){
  local -a names=("$@"); local used="" v
  for n in "${names[@]}"; do
    while :; do
      v="${!n}"; [[ -n "${v:-}" ]] || v=$(rand_port)
      if [[ ! "$used" =~ (^|,)"$v"(,|$) ]]; then
        eval "$n=$v"; used="${used:+$used,}$v"; break
      fi; v=
    done
  done
}
...
status_bar() {
  ...
  if command -v docker >/dev/null 2>&1; then
    if systemctl is-active --quiet docker 2>/dev/null || pgrep -x dockerd >/dev/null; then
      docker_stat="${OK} 运行中"; else docker_stat="${NO} 未运行"; fi
  ...
  else bbr_stat="${NO} 未启用（当前：${cc}，队列：${qd}）"; fi
...
show_result(){
  echo; echo -e "${C_BOLD}${C_GREEN}【完成】${C_RESET} $1"
...
install_docker(){
  ...
  else ok "已安装 Docker"; fi
  command -v jq >/dev/null 2>&1 || pkg_install jq
  command -v openssl >/dev/null 2>&1 || pkg_install openssl
  command -v ss >/dev/null 2>&1 || pkg_install iproute2 || pkg_install iproute || true
...
########################  生成配置  ########################
write_config(){
  load_ports; gen_reality_keys; gen_cert
...
share_links(){
  source "$SB_DIR/ports.env"
  source "$SB_DIR/account.env"
  local sni_enc pbk_enc sid_enc
  sni_enc=$(urlenc "$REALITY_SNI"); pbk_enc=$(urlenc "$REAL_PUB"); sid_enc=$(urlenc "$SHORT_ID")
...
########################  动作  ########################
deploy(){ install_docker; write_config; docker_run; show_result "部署完成！"; show_status_and_links_then_exit; exit 0; }
restart_container(){ docker restart "$CONTAINER_NAME" >/dev/null 2>&1 || err "容器未安装"; }
update_image(){
  install_docker
  local old new
  ...
  if [[ -n "$old" && -n "$new" && "$old" != "$new" ]]; then docker_run; ok "已更新为最新镜像并重启。"
  else ok "已是最新镜像。"; fi
}
self_update(){
  ...
  if ! cmp -s "$0" "$TOOLS_DIR/sing-box-plus.new"; then
    mv "$TOOLS_DIR/sing-box-plus.new" "$0"; chmod +x "$0"; ok "脚本已更新。请重新运行。"
  else rm -f "$TOOLS_DIR/sing-box-plus.new"; ok "脚本已是最新版。"; fi
}
reassign_ports(){ randomize_ports; write_config; docker_run; }
enable_bbr(){
...
uninstall_all(){ docker rm -f "$CONTAINER_NAME" >/dev/null 2>&1 || true; rm -rf "$SB_DIR"; }
show_status_and_links_then_exit(){
  echo; echo -e "${C_BOLD}配置目录: ${C_RESET}$SB_DIR"
  echo -e "${C_BOLD}服务器IP: ${C_RESET}$IPV4_ADDR"; echo
  show_params; share_links
}
...
menu(){
  while true; do
    ...
    case "$opt" in
      ...
      6) reassign_ports; show_result "端口已全部更换" "到“查看状态 & 分享链接”获取最新链接"; read -p "按回车返回菜单..." _ || true ;;
